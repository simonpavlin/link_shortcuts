...
## Known Lint Violations (pre-existing)

These exist in current code and should be fixed when those files are next touched:

- `HomePage.tsx` — multi-comp (EvalTrace helper) + conditional useState
- `RuleList.tsx` — multi-comp (PendingRows helper)
- `RuleRow.tsx` — multi-comp (PatternField helper)
